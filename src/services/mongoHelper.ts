...
class MongoDBHelper {
  ...
  public get(params) {
    return new Promise((resolve, reject) => {
      ...
      if (params.populate) {
        for (const i of params.populate) {
          query.populate(i)
        }
      }
  ...
  public update(params, data) {
    return new Promise((resolve, reject) =>
      this.mongoDBModel.findOneAndUpdate(params.conditions, { $set: data }, { new: true }, (error, response) => {
        if (error) {
          return reject(error)
        }
   ...
   * @returns {Promise}
   */
  public push(params, data) {
    return new Promise((resolve, reject) =>
      this.mongoDBModel.update(params.conditions, { $push: data }, (error, response) => {
        if (error) {
          return reject(error)
        }
        return resolve(response)
   ...
   * @returns {Promise}
   */
  public removeOneFromArray(params, data) {
    return new Promise((resolve, reject) =>
      this.mongoDBModel.update(params.conditions, { $pull: data }, { multi: true }, (error, response) => {
        if (error) {
          return reject(error)
        }
        return resolve(response)
